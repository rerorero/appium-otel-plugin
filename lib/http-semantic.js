...
const { Semantic, Delegator } = require('./semantic.js');
const { telemetry, capabilitiesToAttributes } = require('./telemetry.js');
...
class Shared {
  ...
  bindSessionAttributesToSpan(span, sessionId) {
    ...
    span.setAttributes(data.getAttributes());
  }
 ...
 */
class HTTPSemanticDelegater extends Delegator {

  constructor(attributesInCaps, sharedInstance = shared) {
    super();
...
function newHttpInstrumantationConfig(sharedInstance = shared) {
  return {
...
class HTTPSemantic extends Semantic {
  newDelegater(conf, tracer) {
    return new HTTPSemanticDelegater(conf.attributesInCaps);
  };
  ...
  registerOption(provider) {
    return {
      instrumentations: [
        ...
        new HttpInstrumentation(newHttpInstrumantationConfig(shared)),
      ],
    };
  };
}

export { HTTPSemantic, newHttpInstrumantationConfig, HTTPSemanticDelegater, extractSessionIdFromUrl, Shared, SessionData };
